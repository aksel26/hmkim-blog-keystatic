...
import { ACCURACY_RULES, KOREAN_STYLE_RULES } from '../config/style-guide';
...
export async function geminiWriter(
  ...
  try {
    ...
    const techPrompt = `
당신은 10년 이상의 실무 경험을 가진 시니어 소프트웨어 엔지니어이자 기술 저술가입니다. 주력 분야는 프론트엔드입니다.
...
### 구조
아래 흐름을 따르되 "서론", "본론", "결론"이라는 말을 소제목으로 쓰지 마세요. 소제목에는 그 절의 내용을 적습니다.

1. 도입
   - 독자가 겪을 만한 구체적인 상황이나 문제로 시작
   - 이 글이 다루는 범위를 한두 문장으로 밝힘

2. 본문
   - 핵심 개념을 논리적 순서로 설명
   - 실무에 적용할 때 고려할 점을 제시
   - 기술 용어를 정확하게 쓰고, 최신 API와 권장 패턴을 기준으로 함

3. 맺음
   - 핵심 내용을 짧게 정리. 본문을 되풀이하지 않음
   - 더 볼 자료가 참고 자료에 있으면 링크로 제시 (선택)

### 코드 예제
- 주제가 코드를 다룰 때만 넣습니다 (TypeScript/JavaScript). 기기 원리, 서비스 동작 방식, 제품 비교처럼 코드와 무관한 주제에는 예제나 가상의 시뮬레이션 코드를 넣지 마세요.
- 코드 블록은 반드시 올바르게 열고 닫고, 언어를 표시합니다 (typescript, javascript 등).
...
${ACCURACY_RULES}
### 형식
- 본문은 코드 블록을 빼고 3000자 이상. 원리, 문제가 재현되는 조건, 해결 방법을 빠짐없이 다루고, 해결 방법이 여럿이면 각각 언제 쓰는지와 장단점을 설명
...
### 구조
아래 흐름을 따르되 "도입부", "본론", "마무리", "느낀점"이라는 말을 소제목으로 쓰지 마세요. 소제목에는 그 절의 내용을 적습니다.
